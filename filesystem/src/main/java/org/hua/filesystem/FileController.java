...
public class FileController {
  ...
  @GetMapping("api/file/{fullname}/download")
  public HttpEntity<byte[]> download(@PathVariable String fullname) {
    InputStream is = null;
    try {
      ...
      //创建临时文件
      byte[] imageByteArray = download.getBase64file() == null ? download.getFileByte()
          : Base64Util.decodeImage(download.getBase64file());
  ...
  @GetMapping("api/file/{fullname}/preview")
  public HttpEntity<byte[]> lookup(@PathVariable String fullname) {
    InputStream is = null;
    try {
      FileParam file = new FileParam();
      file.setFullname(fullname);
